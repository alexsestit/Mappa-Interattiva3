...
function style(feature) {
    const value = feature.properties.abs_value;
    return {
...
// Carica il GeoJSON e i dati JSON
Promise.all([
    fetch('./NUTS_RG_20M_2021_4326_LEVL_3.geojson').then(response => response.json()),
    fetch('./dati_mappa_abs.json').then(response => response.json())
]).then(([geojson, data]) => {
    // Unisci i dati
    ...
    geojson.features.forEach(feature => {
        ...
        if (value) {
            feature.properties.abs_value = value;
        } else {
            feature.properties.abs_value = null;
        }
    });
    ...
    L.geoJSON(geojson, {
        ...
        onEachFeature: function(feature, layer) {
            ...
            const value = feature.properties.abs_value;
            const popupContent = `<b>${name}</b><br/>Furti: ${value !== null ? value : 'Dato mancante'}`;
    ...
    legend.onAdd = function (map) {
        ...
        div.innerHTML += '<h4>Numero di Furti</h4>';
