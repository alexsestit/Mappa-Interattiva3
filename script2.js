// Funzione per ottenere il colore in base al valore (tasso di furto)
function getColor(d) {
    return d > 3 ? '#800026' :
           d > 2.5 ? '#BD0026' :
           d > 2 ? '#E31A1C' :
...
Promise.all([
    ...
    fetch('./dati_mappa_veh.json').then(response => response.json())
]).then(([geojson, data]) => {
    // Unisci i dati
    ...
    L.geoJSON(geojson, {
        ...
        onEachFeature: function(feature, layer) {
            ...
            const value = feature.properties.rate_time;
            const popupContent = `<b>${name}</b><br/>Tasso di furti: ${value !== null ? value.toFixed(3) : 'Dato mancante'}`;
    ...
    legend.onAdd = function (map) {
        ...
        const grades = [0, 0.25, 0.5, 1, 1.5, 2, 2.5, 3];
